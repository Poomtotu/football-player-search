...
for path in [root / 'data' / 'players.json', root / 'data' / 'mock_players.json']:
    ...
    for p in players:
        ...
        path_text = ' → '.join(history) if history else 'ยังไม่มีข้อมูลเส้นทางอาชีพ'
        p['bio'] = f'{name} เป็นนักฟุตบอลอาชีพ ปัจจุบันลงเล่นให้ {team} ใน {league} และเป็นตัวแทนทีมชาติ {nation} เส้นทางสโมสร: {path_text}.'
        q = quote_plus(name)
        p['social_links'] = {
    ...
    print('updated', path.name, len(players))
